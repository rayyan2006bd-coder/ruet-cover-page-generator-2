...
export function Editor() {
  ...
  const identityLocks = useAtomValue(activeIdentityLocksAtom);
  ...
  return (
    ...
    >
      ...
      <TabsContent value="student" className={tabContentClass}>
          ...
          <>
            <div className="grid gap-4 sm:grid-cols-2">
              <FormItem label="Student ID">
                <TextInput
                  atom={editorStore.studentID}
                  data-testid="student-id-input"
                  disabled={identityLocks.includes('roll')}
                />
              </FormItem>
            ...
            <FormItem label="Full Name">
              <TextInput
                atom={editorStore.studentName}
                data-testid="student-name-input"
                disabled={identityLocks.includes('name')}
              />
            </FormItem>
          </>
        )}
        <FormItem label="Department">
          <Combobox
            ...
            disabled={identityLocks.includes('department')}
          />
        </FormItem>
      ...
      <TabsContent value="teacher" className={tabContentClass}>
        ...
        <FormItem label="Teacher Name">
          <TeacherName
            ...
            stableKeyAtom={editorStore.teacherStableKey}
            sourceAtom={editorStore.teacherSource}
            inputTestId="primary-teacher-input"
          />
        </FormItem>
        {!!teacherName && (
          ...
          <>
            ...
            <FormItem label="Second Teacher Name">
              <TeacherName
                ...
                stableKeyAtom={editorStore.secondTeacherStableKey}
                sourceAtom={editorStore.secondTeacherSource}
                inputTestId="second-teacher-input"
              />
            </FormItem>
            {!!secondTeacherName && (
              <>
      ...
      <TabsContent value="settings" className={tabContentClass}>
        ...
        />
        <FilenameSettings />
        <FormItem label="PDF export mode">
          <SelectInput<string>
            name="PDF export mode"
            atom={editorStore.pdfExportMode}
            options={[
...
function FilenameSettings() {
  const cover = useAtomValue(coverFormDataAtom);
  return (
    <div className="space-y-2 rounded-lg border p-3 not-prose">
      <FormItem label="PDF filename pattern">
        <TextInput atom={editorStore.filenamePattern} />
        <FormDescription>
          Tokens: {'{department}'}, {'{courseCode}'}, {'{roll}'}, {'{type}'},{' '}
          {'{itemNumber}'}, {'{courseTitle}'}, {'{studentName}'}, {'{date}'}.
          Reset restores {DEFAULT_FILENAME_PATTERN}.
        </FormDescription>
      </FormItem>
      <p className="break-all text-muted-foreground text-xs">
        Preview: {buildSmartFilename(cover)}
      </p>
    </div>
  );
}
