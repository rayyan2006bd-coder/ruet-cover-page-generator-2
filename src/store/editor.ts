...
const courseNo = atom(
  ...
  (get, set, courseNo: string | typeof RESET) => {
    ...
    if (courseNo.length >= 3) {
      ...
      // }
      if (courseNo.length >= 7) {
        const titleAtRequest = get(_courseTitleAtom);
        idbKeyVal.get(courseNo, courseTitleIDBStore).then((x) => {
          if (
            x &&
            get(_courseNoAtom) === courseNo &&
            get(_courseTitleAtom) === titleAtRequest
          ) {
            set(_courseTitleAtom, x);
          }
        });
      }
    }
...
export default {
  ...
  activeProfileId: stringItem('active-profile-id', ''),
  studentDepartment,
  studentName,
  studentID,
  studentSection,
  studentGroup: stringItem('student-group', ''),
  studentSessionValue: stringItem('student-session-value', ''),
  studentSeriesValue: stringItem('student-series-value', ''),
  courseNo,
  courseTitle,
  courseStableKey: stringItem('course-stable-key', ''),
  courseDepartmentKey: stringItem('course-department-key', ''),
  courseDepartment: stringItem('course-department', ''),
  courseSource: stringItem('course-source', 'manual'),
  type,
  coverNo,
  ...
  teacherStableKey: stringItem('teacher-stable-key', ''),
  teacherSource: stringItem('teacher-source', 'manual'),
  ...
  secondTeacherStableKey: stringItem('second-teacher-stable-key', ''),
  secondTeacherSource: stringItem('second-teacher-source', 'manual'),
  templateStableKey: stringItem('template-stable-key', ''),
  templateVersion: stringItem('template-version', ''),
  templateName: stringItem('template-name', 'General RUET cover'),
  templateApproved: booleanItem('template-approved', false),
  filenamePattern: stringItem(
    'filename-pattern',
    '{department}-{courseCode}_{roll}_{type}-{itemNumber}.pdf',
  ),
  pdfExportMode: stringItem('pdf-export-mode', 'standard'),
